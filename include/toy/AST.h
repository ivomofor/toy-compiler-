#ifndef TOY_AST_H
#define TOY_AST_H

#include <memory>
#include <string>
#include <vector>

namespace toy {

    enum class ASTNodeKind {
        Program,
        FunctionDecl,
        ReturnStmt,
        PrintStmt,
        VariableDecl,
        IfStmt,
        WhileStmt,
        CallExpression,
        IntegerLiteral,
        VariableReference,
        BinaryExpression
    };

    class ASTNode {
    public:
        explicit ASTNode(ASTNodeKind kind)
            : kind(kind) {}

        virtual ~ASTNode() = default;

        ASTNodeKind getKind() const {
            return kind;
        }

    private:
        ASTNodeKind kind;
    };

    class Statement : public ASTNode {
    public:
        explicit Statement(ASTNodeKind kind)
            : ASTNode(kind) {}

        virtual ~Statement() = default;
    };

    class Expression : public ASTNode {
    public:
        explicit Expression(ASTNodeKind kind)
            : ASTNode(kind) {}

        virtual ~Expression() = default;
    };
    class Program : public ASTNode {
    public:
        Program()
            : ASTNode(ASTNodeKind::Program) {}

        std::vector<std::unique_ptr<ASTNode>> declarations;
    };

    class FunctionDecl : public ASTNode {
    public:
        explicit FunctionDecl(std::string name)
            : ASTNode(ASTNodeKind::FunctionDecl),
              name(std::move(name)) {}

        std::string name;
        std::vector<std::string> parameters;
        std::vector<std::unique_ptr<Statement>> body;
    };

    class ReturnStmt : public Statement {
    public:
        explicit ReturnStmt(std::unique_ptr<Expression> value)
            : Statement(ASTNodeKind::ReturnStmt),
              value(std::move(value)) {}

        std::unique_ptr<Expression> value;
    };

    class PrintStmt : public Statement {
    public:
        explicit PrintStmt(std::unique_ptr<Expression> value)
            : Statement(ASTNodeKind::PrintStmt),
              value(std::move(value)) {}

        std::unique_ptr<Expression> value;
    };
    
    class VariableDecl : public Statement {
    public:
        VariableDecl(std::string name, std::unique_ptr<Expression> initializer)
            : Statement(ASTNodeKind::VariableDecl),
              name(std::move(name)),
              initializer(std::move(initializer)) {}

        std::string name;
        std::unique_ptr<Expression> initializer;
    };
    class IfStmt : public Statement {
    public:
        IfStmt(
            std::unique_ptr<Expression> condition,
            std::vector<std::unique_ptr<Statement>> thenBody)
            : Statement(ASTNodeKind::IfStmt),
              condition(std::move(condition)),
              thenBody(std::move(thenBody)) {}

        std::unique_ptr<Expression> condition;
        std::vector<std::unique_ptr<Statement>> thenBody;
    };
    class WhileStmt : public Statement {
    public:
        WhileStmt(
            std::unique_ptr<Expression> condition,
            std::vector<std::unique_ptr<Statement>> body)
            : Statement(ASTNodeKind::WhileStmt),
              condition(std::move(condition)),
              body(std::move(body)) {}

        std::unique_ptr<Expression> condition;
        std::vector<std::unique_ptr<Statement>> body;
    };
    class CallExpression : public Expression {
    public:
        CallExpression(
            std::string callee,
            std::vector<std::unique_ptr<Expression>> arguments)
            : Expression(ASTNodeKind::CallExpression),
              callee(std::move(callee)),
              arguments(std::move(arguments)) {}

        std::string callee;
        std::vector<std::unique_ptr<Expression>> arguments;
    };
    class IntegerLiteral : public Expression {
    public:
        explicit IntegerLiteral(int value)
            : Expression(ASTNodeKind::IntegerLiteral),
              value(value) {}

        int value;
    };
    class VariableReference : public Expression {
    public:
        explicit VariableReference(std::string name)
            : Expression(ASTNodeKind::VariableReference),
              name(std::move(name)) {}

        std::string name;
    };
    class BinaryExpression : public Expression {
    public:
        BinaryExpression(
            char op,
            std::unique_ptr<Expression> left,
            std::unique_ptr<Expression> right)
            : Expression(ASTNodeKind::BinaryExpression),
              op(op),
              left(std::move(left)),
              right(std::move(right)) {}

        char op;
        std::unique_ptr<Expression> left;
        std::unique_ptr<Expression> right;
    };
}

#endif